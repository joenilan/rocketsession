...
use serde_json::json;
use std::{
    collections::VecDeque, convert::Infallible, net::SocketAddr, path::PathBuf, sync::Arc,
    time::Duration,
};
...
use crate::session::{SharedSnapshot, TextOverlayElement, TrackerCmd};
...
async fn get_text_stat(
    State(state): State<AppState>,
    axum::extract::Path(stat): axum::extract::Path<String>,
) -> impl IntoResponse {
    let snap = state.shared.read().await.clone();
    match format_text_stat(&snap, &stat) {
        Some(value) => {
        ...
        None => (StatusCode::NOT_FOUND, "Unknown stat").into_response(),
    }
}
...
async fn post_network_access(
    ...
    let _ = state
        .cmd_tx
        .send(TrackerCmd::SetAllowDualPc(body.allow_dual_pc));
...
async fn get_stats_api_config(Query(q): Query<StatsApiConfigQuery>) -> impl IntoResponse {
...
async fn post_overlay_config(
    State(state): State<AppState>,
    axum::Json(body): axum::Json<OverlayConfigBody>,
) -> impl IntoResponse {
    let _ = state.cmd_tx.send(TrackerCmd::SetOverlayConfig {
        overlay_mode: body.overlay_mode,
        text_overlay_elements: body.text_overlay_elements,
    });
    tokio::time::sleep(Duration::from_millis(20)).await;
    let snap = state.shared.read().await.clone();
    json_response(StatusCode::OK, &snap)
...
async fn serve_static(State(state): State<AppState>, uri: axum::http::Uri) -> impl IntoResponse {
...
fn format_streak(streak: i32) -> String {
    if streak > 0 {
        ...
        format!("L{}", streak.unsigned_abs())
    } else {
        "0".to_string()
    }
}

fn format_text_stat(snap: &crate::session::SessionSnapshot, stat: &str) -> Option<String> {
    let totals = &snap.totals;
    match stat {
        "wins" => Some(totals.wins.to_string()),
        "losses" => Some(totals.losses.to_string()),
        "games" => Some(totals.games.to_string()),
        "streak" => Some(format_streak(totals.streak)),
        "winrate" => Some(if totals.games > 0 {
            format!(
                "{}%",
                ((totals.wins as f64 / totals.games as f64) * 100.0).round() as u32
            )
        } else {
            "0%".to_string()
        }),
        "goals" => Some(totals.goals.to_string()),
        "assists" => Some(totals.assists.to_string()),
        "saves" => Some(totals.saves.to_string()),
        "shots" => Some(totals.shots.to_string()),
        "demos" => Some(totals.demos.to_string()),
        "ballHits" => Some(totals.ball_hits.to_string()),
        "strongestHit" => Some((totals.strongest_hit.round() as u32).to_string()),
        "trackedPlayer" => Some(
            snap.tracked_player
                .as_ref()
...
pub fn build_router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .route("/text/{stat}", get(get_text_stat))
        ...
        .route(
            "/api/stats-api-config/disable",
            post(post_stats_api_disable),
        )
        .route("/api/overlay-settings", post(post_overlay_settings))
        .route("/api/overlay-config", post(post_overlay_config))
